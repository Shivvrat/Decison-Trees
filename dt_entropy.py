...
def Information_Gain_Heuristic(examples, attributes, target_attribute):
    ...
    for each_attribute in attributes:
        ...
        try:
            ...
            # Here I used the try except clause because there can be a case where we don't have the counts list to be
            # of length 2
            i = 0
        # Since I have hardcoded the array_after_division arrays we will try to the get values for 0 first.
        ...
        for each in list_of_row_names:
            # Here we just append the values of the instances which have the same each_attribute value
        ...
        # Here I will use try except since if the target_attribute have only one unique value then it will give an
        # error if we try to use the second index (i.e. 1). and if we have only one unique value then our impurity
...
and make the tree while doing the training. We have chosen information gain as the best attribute selection criteria
"""
...
def decision_tree_construction(examples, target_attribute, attributes):
    ...
    """
    # This is the first base condition of the algorithm. It is used if the attributes variable is empty,
    # then we return the single-node tree Root, with label = most common value of target_attribute in examples The
    # second base condition for the recursion when we check if all the variables are same or not in the node and if
    # they are same then we return that value as the node
...
def decision_tree_predict(tree, testing_example, max_value_in_target_attribute):
    ...
    :return:
    """
    # We take each attribute for the data point anc check if that attribute is the root node for the tree we are on
...
def decision_tree_accuracy(test_y, predicted_y):
    ...
    """
    # We have converted the y value to a list so that we can compare it with the predicted_y
...
def main_dtree_with_entropy(train_x, train_y, test_x, test_y):
    """
